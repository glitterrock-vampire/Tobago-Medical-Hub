/**
 * Vercel build script — bundles the Express API into a single ESM file
 * that Vercel's @vercel/node runtime can serve as a serverless function.
 *
 * Output: <monorepo-root>/api/index.mjs
 *
 * Usage (called by vercel.json buildCommand):
...
await esbuild({
  // Named entry so the output file is always api/index.mjs
...
console.log(`✓ Vercel API bundle → ${path.join(outDir, "index.mjs")}`);
